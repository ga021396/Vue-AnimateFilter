  ...
  <div>
      ...
      <div class="introduction">
        <div class="name">
          ...
          <font-awesome-icon
            ...
            icon="star"
            class="icon-heart"
            :class="{'icon-active':getMyFavorite(item)}"
            @click="setMyFavorite(item)"
          />
        </div>
        ...
        <div class="TypeSection">
          <span
            class="Typetag"
            v-for="(type,index) in getType(item.type) "
            :key="index"
    ...
    <div class="pageBox">
      <div
        ...
        class="page"
        v-for="(item,index) in this.pageSize"
        :key="index"
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      if (this.searchContent.trim() !== "") {
        filterList = filterList.filter(
          item =>
            item.title.toLowerCase().indexOf(this.searchContent.toLowerCase()) >
            -1
        );
      }
    ...
    checkStory(story) {
      ...
      if (story.length === 0)
        return "ストーリーはまだ見つかりません、少々お待ちください。ストーリーはまだ見つかりません、少々お待ちください。";
      else return story;
    },
    setMyFavorite(anime) {
      //this function save/remove your favorite anime to firebase
      const isMyfire = this.userFireData.favorites.findIndex(
